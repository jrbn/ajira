 ...
 * to store tuples in the main memory and to manage
 * caching/storing on the disk when the size exceeds 
 * the maximum limit. Basically, it contains a buffer
 * and a map over the files that were used to spill
 * tuples on the disk.  
 */
public class Bucket {

	/**
	 * Keeps information about the content (tuples)
	...
	/**
	 * Initialization method for the bucket's private/public 
	 ...
	 * into the local one. Instead of doing one add per each new tuple, we 
	 * copy the entire content of the new container into the bucket.
	 * Mainly, this method is used to add a whole chunk transferred from a 
	 * remote bucket.
	 * 
	 ...
	 * 
	 * @return 
	 * 		true/false whether the bucket has enough bytes to transfer
	 * 		or there are some cached files and we prefer to empty the buffer
	 */
	...
	/**
	 * Method that calls cacheBuffer() directly over the buffer, 
	...
	/**
	 * This method is used to internally check if all the 
	 * send-receive operations performed over the bucket 
	 * are finished.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * This method is used to check whether the element
	 * given as a parameter is less/greater then the last 
	 * element (min) from the sorted list used for merging 
	 * the bucket's tuples.
	 * 
	 * @param element 
	 * 		The element (sequence of bytes) to compare with
	 * @return
	 * 		True/false, if the element is lees or not than
	 * 		the sorted list's minimum 
	 */
	 ...
	 * 		checked if can copied into the buffer
	 * @param tmpBuffer
	 * 		The destination buffer - where to copy the entire
	 * 		cached file if the conditions are passed
	 * @param minimum
	 * 		The minimum element from the file which we attempt
	 * 		to copy into the buffer (if the conditions are
	 * 		fulfilled)
	 * @return
	 * 		True/false if the copy attempt was successful or not
	 * @throws Exception
	 */
	...
	/**
	 * Registers the bucket as being finished and notifies the
	 * iterator (it marks as ready).
	 * 
	 * @param notifier
	 * @param iter
	 */
	 ...
	 * This method is used to remove a chunk from the buffer in order 
	 * to be transferred or written to the final result file (by calling
	 * the bucketIterator).
	 * To remove a chunk and to still have an ordered sequence of tuples
	 * we first have to merge tuples from the buffer with tuples from
	 * content cached in files (if any) up until we managed to fill
	 * the chunk to its maximum size. 
	 *  
	 * @param tmpBuffer
	 * 		The chunk removed from the bucket (globally sorted) 
	 * @return
	 * 		True/false, depending on whether the bucket still contains
	 * 		tuples to remove (buffer + files) or not
	 */
	...
	/**
	 * Method to update counters about the send-receive status
	 * (if last sequence/chunk was received or not).
	 * 
	 * @param sequence
	 * 		Sequence number
	 * @param lastSequence
	...
	/**
	 * This method is used to update counters with information
	 * about the current chain status.
	 * 
	 * @param idChain
	 * 		Current chain id
	 * @param idParentChain
	 ...
	 * @param isResponsible
	 * 		True/false, whether the current chain is
	 * 		a root chain or not
	 * @throws IOException
	 */
	...
	/**
	 * This method is used to wait until all the cachers (the 
	 * threads that caches into files the content of a buffer) 
	 * have finished spilling to disk -- calling notify.   
	 */
	 ...
	 * 		otherwise we enter into wait() mode
	 */
