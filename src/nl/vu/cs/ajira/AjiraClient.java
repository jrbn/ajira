...
public class AjiraClient {
	 ...
	 * properties file, and submits a job to the cluster.
	...
	public AjiraClient(String filename, Job job) throws IOException {
		...
		// Determine which node to connect to, and create a receive port to receive
	...
	public boolean getResult(List<Tuple> v) throws JobFailedException {
		...
		try {
			...
			if (! success) {
				try {
					...
					try {
						...
						throw new JobFailedException("Job failed, but could not obtain the reason", e1);
					}
					throw e;
				} finally {
					close();
			...
			WritableContainer<TupleSerializer> tmpBuffer = new WritableContainer<TupleSerializer>(Consts.TUPLES_CONTAINER_BUFFER_SIZE);
	...
	private boolean getMoreResults(List<Tuple> v) throws JobFailedException {
		if (closed || ! getResultCalled) {
			...
			throw new JobFailedException("getMoreResults called incorrectly", null);
		}
		try {
